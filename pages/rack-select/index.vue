...
import { selectLayerInd, handleRackClickInd } from '@/composables/rackClick';
...
const selectLayer = (layer) => {
  selectLayerInd(layer, data);
};

const statusChangeClick = (rackObj) => {
  applyStatusChangeEffect(rackObj, data, 'individual');
};

const handleRackClick = (obj) => {
  handleRackClickInd(obj, data);
};

const onClickTaskFunc = async () => {
  await handleRackTaskConfirm(
    data,
    task.value,
    router,
    taskStore,
...
const sendTaskConfirm = async () => {
  await confirmAndSendRackTask(task.value, router, 'individual', data);
};
...
onMounted(async () => {
  ...
  prepareOkibaDisplayState(data, task.value, 'individual', (startOrGoal) => {
    handleRackTypeShowing(
      data.allUiRacks,
      task.value.deliveryTask.deliveryType,
  ...
  task.value.deliveryTask.goal.goalRacks.forEach((goalRack) => {
    ...
    if (
      ...
      task.value.deliveryTask.deliveryType == 2
    ) {
      data.wholeGoalRacks += 1;
    }
  });
