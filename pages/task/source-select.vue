...
const handleArea = (area) => {
  data.disabled = false;
  task.value.deliveryTask.goal.goalArea == ''
    ? taskStore.setTaskProperty('deliveryTask.start.startArea', area)
    : taskStore.setTaskProperty('deliveryTask.goal.goalArea', area);
  if (area != '') {
...
const toNextPage = () => {
  if (data.disabled) {
    return null;
  } else router.push('/task/num-input');
};

onMounted(async () => {
  taskStore.setTaskProperty('deliveryTask.start.startArea', '');
